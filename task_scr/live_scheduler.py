...
class LiveScheduer:
    def __init__(self, config, use_drive, use_service_account, gdrive_setting_path=""):
        ...
        self.editing_dic = (
            {}
        )  # {"thread-id":{"schedule":{},status=["choice","edit","end"],day="",timestamp:""}}
    ...
    def get_icons(self, icon_paths, grid_name):
        ...
        for key, val in icon_paths.items():
            if grid_name == "number_img":
                ...
                if self.use_drive:
                    icon_objects[key] = self.image_resize(
                        self.get_image(val.split("/")[5]), grid_name
                    )
                else:
                    icon_objects[key] = self.image_resize(
                        self.get_image(val), grid_name
                    )
        return icon_objects
        ...
        for line in text.split("\n")[1:]:
            ...
            if len(line_list) == 2:  # 日付
                ...
                if line_list[-1] == "liver":
                    ...
                    if len(tmp) == 3:
                        result[tmp[0]] = datetime.datetime.strptime(
                            "{}:{}".format(tmp[1], tmp[2]), "%H:%M"
                        )
                    else:
                        result[tmp[0]] = tmp[1]
        ...
        if thread.id in self.editing_dic:  # すでに編集中の時
            if self.editing_dic[thread.id]["status"] == "choice":  # 日付選択直後
                if text in self.editing_dic[thread.id]["schedule"]:
                    choiced_schedule = self.schedule_json_to_markdown(
                        int(text), self.editing_dic[thread.id]["schedule"][text]
                    )
                ...
                await thread.send(
                    self.schedule_all_json_to_markdown(
                        self.editing_dic[thread.id]["schedule"]
                    )
                )
