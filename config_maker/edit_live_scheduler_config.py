...
from common import (
    str_to_bool,
    wrapper_edit_live_scheduler_img_config,
    wrapper_edit_live_scheduler_grid_config,
    wrapper_end_screen,
)

entrys = {
    "icons": {"entry": [], "img_entry": []},
    "contents": {"entry": [], "img_entry": []},
    "platform": {"entry": [], "img_entry": []},
    "number_img": {"entry": [], "img_entry": []},
}
        ...
        for category, category_val in entrys.items():
            for i in range(len(category_val["entry"])):
                ...
                if val != "" and img != "":  # 空欄は対象外
                    config[category][category_val["entry"][i].get()] = category_val[
                        "img_entry"
                    ][i].get()
...
def dicide_grid_config(window, root, entrys):
    def inner():
        ...
        config = common.config["function"]["live_scheduler"]
        menber_num = len(
            common.config["function"]["live_scheduler"]["files"]["icons"].keys()
        )
...
def edit_live_scheduler_config(root):
    ...
    row = 0
    # 機能のON/OFF
    live_scheduler_label = tk.Label(
        screen, text="live_scheduler機能の読み込みを許可するか（許可：True、不許可：False）"
    )
    ...
    load_img = ttk.Button(
        screen,
        text="ファイル",
        command=load_image(config_val["files"]["format"]),
        state=button_state,
    )
    ...
    load_img = ttk.Button(
        screen,
        text="ファイル",
        command=load_image(config_val["files"]["schedule"]),
        state=button_state,
    )
    load_img.grid(row=row, column=6, columnspan=3, sticky=tk.NW)
    row += 1
    submit = ttk.Button(
        screen,
        text="決定",
        command=decide_live_scheduler_config(root, screen, config_val),
    )
...
def edit_live_scheduler_img_config(root):
    ...
    load_img = ttk.Button(
        screen,
        text="まとめて読みこみ",
        command=load_images(screen, "icons", button_row + 2, col),
        state=button_state,
    )
    ...
    add_line = ttk.Button(
        screen,
        text="1行追加",
        command=add_label_entry(screen, "icons", button_row + 2, col),
    )
    ...
    load_default_val(
        config["files"]["contents"], screen, "contents", button_row + 2, col
    )

    load_img = ttk.Button(
        screen,
        text="まとめて読みこみ",
        command=load_images(screen, "contents", button_row + 2, col),
        state=button_state,
    )
    load_img.grid(row=button_row, column=col, sticky=tk.NW)
    add_line = ttk.Button(
        screen,
        text="1行追加",
        command=add_label_entry(screen, "contents", button_row + 2, col),
    )
    add_line.grid(row=button_row, column=col + 1, sticky=tk.NW)
    ...
    label.grid(row=row, column=col, columnspan=col_span, sticky=tk.NW)
    load_default_val(
        config["files"]["platform"], screen, "platform", button_row + 2, col
    )

    load_img = ttk.Button(
        screen,
        text="まとめて読みこみ",
        command=load_images(screen, "platform", button_row + 2, col),
        state=button_state,
    )
    load_img.grid(row=button_row, column=col, sticky=tk.NW)
    add_line = ttk.Button(
        screen,
        text="1行追加",
        command=add_label_entry(screen, "platform", button_row + 2, col),
    )
    add_line.grid(row=button_row, column=col + 1, sticky=tk.NW)
    ...
    label.grid(row=row, column=col, columnspan=col_span, sticky=tk.NW)
    load_default_val(
        config["files"]["number_img"], screen, "number_img", button_row + 2, col
    )

    load_img = ttk.Button(
        screen,
        text="まとめて読みこみ",
        command=load_images(screen, "number_img", button_row + 2, col),
        state=button_state,
    )
    load_img.grid(row=button_row, column=col, sticky=tk.NW)
    add_line = ttk.Button(
        screen,
        text="1行追加",
        command=add_label_entry(screen, "number_img", button_row + 2, col),
    )
    add_line.grid(row=button_row, column=col + 1, sticky=tk.NW)
    ...
    col += col_span
    submit = ttk.Button(
        screen, text="決定", command=decide_live_scheduler_img_config(root, screen)
    )
...
def show_preview(entrys, error_label):
    ...
    def inner():
        ...
        try:
            error_label[
                "text"
            ] = "※GoogleDriveはpreviewに時間がかかる(30秒～１分程度)。Errorがあればここにでます"
            config = common.config["function"]["live_scheduler"]
            ...
            config["grid"] = calc_new_grid(entrys, config["grid"], menber_num)
            live_scheduler = live_scheduler.LiveScheduer(
                config, use_drive, use_service_account, gdrive_setting_path
            )
...
def edit_live_scheduler_grid_config(root):
    ...
    def make_arrange_button(window, entry, tag):
        for i, button_label in enumerate(grid_move_command):
            if button_label > 0:
                ...
                button = ttk.Button(
                    window,
                    text="+{}".format(button_label),
                    command=move_line(button_label, tag, entry),
                )
                ...
                button.grid(row=row, column=2 + i, sticky=tk.NW)
            button = ttk.Button(
                window, text="入力欄の値を反映", command=on_entry_dicide(tag, entry)
            )
    ...
    canvas.create_line(
        0, top_r_y, img_width, top_r_y, fill="Blue", width=2, tag="top_y"
    )
    ...
    canvas.create_line(
        0, buttom_r_y, img_width, buttom_r_y, fill="red", width=2, tag="buttom_y"
    )
    ...
    canvas.create_line(
        time_x, 0, time_x, img_height, fill="yellow", width=2, tag="time_x"
    )
    ...
    if icons != {}:
        ...
        canvas.create_line(
            liver_x, 0, liver_x, img_height, fill="cyan", width=1, tag="liver_x"
        )
    ...
    canvas.create_line(
        end_x, 0, end_x, img_height, fill="firebrick1", width=1, tag="platform_x"
    )
    ...
    error_label = tk.Label(
        window, text="※GoogleDriveはpreviewに時間がかかる(30秒～１分程度)。Errorがあればここにでます。"
    )
    error_label.grid(row=row + 1, column=1, columnspan=5, sticky=tk.NW)

    submit = ttk.Button(
        window, text="プレビュー)", command=show_preview(entrys, error_label)
    )
    submit.grid(row=row, column=1, sticky=tk.NW)

    submit = ttk.Button(
        window, text="設定を保存する", command=dicide_grid_config(window, root, entrys)
    )
    submit.grid(
        row=row,
        column=2,
