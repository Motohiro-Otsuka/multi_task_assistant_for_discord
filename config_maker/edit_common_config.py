...
def edit_common_config(root):
    ...
    for key, val in config.items():
        ...
        if key == "discord_api_key":
            ...
            label = tk.Label(
                screen, text="google driveにAPI経由でアクセスする場合はTrue、そうでない場合はFalse"
            )
            label.grid(row=row, column=0, columnspan=8, sticky=tk.NW)
            row += 1
            ...
            label = tk.Label(
                screen,
                text="google driveにAPI経由でアクセスする場合、google driveのサービスアカウントを使用する場合はTrue,そうでない場合はFalse（基本Flase）",
            )
            label.grid(row=row, column=0, columnspan=8, sticky=tk.NW)
            row += 1
        elif key == "google_dirive_setting":
            label = tk.Label(
                screen, text="google driveにAPI経由でアクセスする場合、その設定値が書かれたファイルパスを入力する"
            )
            label.grid(row=row, column=0, columnspan=8, sticky=tk.NW)
            row += 1
        # 入力欄の描画
        ...
        # 決定ボタン
    submit = ttk.Button(
        screen, text="決定", command=decide_common_config(root, screen, config_val)
    )
